...
export default createStore<State>({
  ...
  actions: {
    fetchCompanionStatus: async ({ dispatch, commit }) => {
      try {
        const response = await FirestoreService.fetchUserData();
        const compLastOnlineTime = response
          .data()
          .last_online.toDate()
          .getTime();
        ...
        commit("overwrite", {
          key: "companionStatus",
          value:
            currentTime > compLastOnlineTime &&
            compLastOnlineTime > intervalTime
              ? "online"
              : "offline",
        });
      } catch (error) {
    ...
    fetchGameDataById: async ({ dispatch, commit }, gameDataId) => {
      try {
    ...
    fetchPredictions: async ({ dispatch, commit }) => {
      try {
    ...
    setGetDataFlag: async ({ dispatch, commit }) => {
      try {
    ...
    getPredictionForLatestGameData: async ({ dispatch, commit }, parameters) => {
      try {
